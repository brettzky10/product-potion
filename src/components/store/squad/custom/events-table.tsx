...
import { triggerTestWebhook } from "@/lib/actions/store/squad/custom";
...
export function EventsTable({
  ...
  const fetchEvents = useCallback(async () => {
    ...
    const response = await fetch(
      `/api/utils/hookdeck/events?id=${subscription.connection.id}`
    );
  ...
  const fetchAttempts = useCallback(async (id: string) => {
    setLoading(true);
    const response = await fetch(`/api/utils/hookdeck/attempts?id=${id}`);
          ...
          <tbody>
            {eventList.map((event) => (
              <Fragment key={event.id}>
                <tr
                  className="odd:bg-slate-300 cursor-pointer hover:bg-slate-100 h-[30px]"
                  onClick={() => {
                  ...
                  <tr className="border-l-2 border-l-slate-600">
                    <td colSpan={4}>
                      <div className="flex flex-col gap-2 w-full mb-6 pl-2">
                        ...
                        <div
                          className={`relative ${
                            tabSelected === "REQUEST" ? "block" : "hidden"
                          }`}
                        >
                          <WebhookTestButton
                            className=" top-0 right-0 button h-[50px] m-4 z-10"
