...
import { characterCategory, character } from "@prisma/client";
...
import { PREAMBLE, SEED_CHAT } from "@/lib/config";
...
const formSchema = z.object({
  ...
  characterCategoryId: z.string().min(1, {
...
export const CharacterForm = ({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      if (initialData) {
        await axios.patch(`/api/${storeId}/squad/character/${initialData.id}`, values);
      } else {
        await axios.post(`/api/${storeId}/squad/character`, values);
      }
