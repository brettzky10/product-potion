...
import { ShareFeedback } from "@/components/global/share-feedback";
...
export function UserNav({ plan, user, credits}: Props) { //, userDetails
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          className="relative h-7 w-7 rounded-full flex items-center justify-center focus-visible:ring-none border border-gray-300"
        >
      ...
      >
        <DropdownMenuLabel className="flex font-normal mt-1 justify-between items-center">
          ...
          ? <Link href={"/billing"}><Badge variant={"ultimate"} className="mx-1">{plan}</Badge></Link>
          : <Link href={"/billing"}><Badge variant={"standard"} className="mx-1">Free Plan</Badge></Link>
        }
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent side="bottom" align="end">
                <p>You have {credits ?? 0} credits left</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </DropdownMenuLabel>
        ...
        <Link
            href={"/billing"}
            className="hover:underline underline-offset-2 text-sm m-1 items-center"
          >
            <svg
